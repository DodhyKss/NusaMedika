...
- **Controllers**: `app/Http/Controllers/{Modul}/{Menu}/{SubMenu}/{SubMenu}Controller.php` — folder = StudlyCase of the modul name, the menu name, then the sub-menu. Folder leaf **harus PascalCase kata-kata utuh tanpa akronim** (misal `DaftarGawatDarurat`, `ListPasienRanap`, `NasabahPasien`) karena URI/route/controller diturunkan otomatis dari nama leaf tersebut. Examples: `Registrasi/Pendaftaran/DaftarRajal/DaftarRajalController.php`, `Registrasi/Pasien/DaftarPasien/DaftarPasienController.php`, `EMR/EmrDashboard/EmrDashboardController.php`, `Administrator/ManajemenUser/User/UserController.php`.
- **Views**: `resources/views/moduls/{modul}/{menu}/{sub_menu}/...` — folder **sama persis dengan path controller PascalCase** (misal `moduls/Registrasi/Pendaftaran/DaftarRajal/`, `moduls/EMR/Soap/`, `moduls/Administrator/ManajemenMaster/Pegawai/`). Semua view (baik CRUD maupun landing) memakai nama `index/create/edit.blade.php`. Panggil via `view('moduls.{PascalPath}..{File}')` (titik, bukan slash), contoh `view('moduls.RawatInap.Pasien.ListPasienRanap.index')`. Sub-folder partial EMR tetap di `moduls/EMR/PartialForm/`.
