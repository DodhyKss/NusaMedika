...
- **Penyimpanan**: kolom `supplier_id`/`distributor_id` (nullable) di **`pemesanan_detail`** menyimpan supplier/distributor per item (migration `2026_09_13_100018_add_supplier_to_pemesanan_detail_table`), relasi `PemesananDetail::supplier()`/`distributor()`. **Header `pemesanan` TIDAK punya `supplier_id`/`distributor_id`** (kolom dihapus pada migration `2026_09_13_100019_drop_supplier_from_pemesanan_table`) karena satu PO boleh berisi banyak supplier — supplier/distributor hanya disimpan per item. `PenerimaanBarangController` tetap mengisi `penerimaan.supplier_id` (kolom milik `penerimaan` sendiri) dari supplier item pertama (`$pemesanan->details->first()?->supplier_id`); label supplier pada daftar/PO (Setujui Pesanan, Pilih PO, form Penerimaan) dibuat dari `$pemesanan->details->pluck('supplier.nama_supplier')` (unik, digabung koma).
