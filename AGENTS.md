...
- **Penyimpanan**: kolom `supplier_id`/`distributor_id` (nullable) di **`pemesanan_detail`** menyimpan supplier/distributor per item (migration `2026_09_13_100018_add_supplier_to_pemesanan_detail_table`), relasi `PemesananDetail::supplier()`/`distributor()`. Header **`pemesanan.supplier_id`/`distributor_id`** dipertahankan sebagai **ringkasan = nilai item pertama** (migration `100017` untuk `distributor_id`; `100018` juga menjadikan `pemesanan.supplier_id` nullable) karena alur Penerimaan Barang memakai `$pemesanan->supplier_id` (`PenerimaanBarangController` line 78 + view). Edit PO legacy yang detailnya belum berisi supplier per baris otomatis prefill dari header (`headerSupplier`/`headerDistributor`) agar tetap bisa disimpan.
