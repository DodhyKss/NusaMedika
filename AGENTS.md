...
- **Controllers**: `app/Http/Controllers/{Modul}/{Menu}/{SubMenu}/{SubMenu}Controller.php` — folder = StudlyCase of the modul name, the menu name, then the sub-menu. Folder leaf **harus PascalCase kata-kata utuh tanpa akronim** (misal `DaftarGawatDarurat`, `ListPasienRanap`, `NasabahPasien`) karena URI/route/controller diturunkan otomatis dari nama leaf tersebut. **Leaf = identitas unik sub_menu**: nama folder (segmen terakhir `file_sub_menu`) **wajib unik di seluruh app** — URI = `Str::snake(leaf)`, jadi dua sub_menu dengan leaf sama akan bentrok (URI/route/controller ganda). Leaf harus NamaBenda + kata pembeda (misal `ListPasienDokter`, `ListPasienRanap`, `ListPasienGawatDarurat` berbagi kata dasar "Pasien" tapi leaf beda); JANGAN pakai nama menu/modul saja sebagai leaf dan JANGAN buat dua sub_menu dengan leaf sama (kalau memang halaman sama, gabung jadi satu sub_menu). Examples: `Registrasi/Pendaftaran/DaftarRajal/DaftarRajalController.php`, `Registrasi/Pasien/DaftarPasien/DaftarPasienController.php`, `EMR/EmrDashboard/EmrDashboardController.php`, `Administrator/ManajemenUser/User/UserController.php`.
- **Views**: `resources/views/moduls/{modul}/{menu}/{sub_menu}/...` — **path folder harus IDENTIK persis dengan path controller** (nama segmen & huruf besar/kecil sama, misal `file_sub_menu='Registrasi/Pasien/DaftarPasien'` → view `resources/views/moduls/Registrasi/Pasien/DaftarPasien/` **dan** controller `App\Http\Controllers\Registrasi\Pasien\DaftarPasien\`). Karena `derive()` memakai `file_sub_menu` yang sama untuk membentuk namespace controller dan path view, kalau keduanya berbeda maka salah satunya tidak ditemukan (class not found / view 404). Semua view (baik CRUD maupun landing) memakai nama `index/create/edit.blade.php`. Panggil via `view('moduls.{PascalPath}.{File}')` (titik, bukan slash), contoh `view('moduls.RawatInap.Pasien.ListPasienRanap.index')`. Sub-folder partial EMR tetap di `moduls/EMR/PartialForm/`.
- **`sub_menu.file_sub_menu`** sekarang menyimpan **path base view PascalCase** (folder view, misal `Registrasi/Pasien/DaftarPasien`, `Administrator/ManajemenMaster/Bagian`), bukan slug URI. Dari nilai inilah URI, nama route, controller, dan URL sidebar **diturunkan otomatis** (lihat section *Auto-route sub_menu*). Gunakan `'#'` hanya untuk sub-menu yang butuh id dinamis (EMR forms).
...
- **Penamaan normal & bebas override**: karena semua folder leaf sudah dinormalisasi menjadi PascalCase kata-kata utuh tanpa akronim (lihat section *Modular structure*), derivasi murni struktural — **tidak ada tabel OVERRIDES per `sub_menu_id`**. Aturan: `uri` = `Str::snake(leaf)` (misal `DaftarPasien`→`daftar_pasien`, `DaftarGawatDarurat`→`daftar_gawat_darurat`, `ListPasienDokter`→`list_pasien_dokter`, `ReferensiBagianId`→`referensi_bagian_id`); `controller` = `App\Http\Controllers\{namespace}\{leaf}Controller` (foldernya harus sama persis dengan `file_sub_menu`). **Gotcha `Str::snake`**: nama folder/class harus full-word (bukan akronim) karena `Str::snake('DaftarIGD')`=`daftar_i_g_d` dan `Str::snake('ListPasienIGD')`=`list_pasien_i_g_d` — makanya folder IGD dieja penuh (misal sub-menu "Registrasi IGD" → leaf `DaftarGawatDarurat`, "List Pasien IGD" → leaf `ListPasienGawatDarurat`).
  - `param` route = `Str::snake(segmen terakhir uri)` (mengikuti konvensi `Route::resource`, agar panggilan `route('...', ['wilayah' => $id])` yang memakai key model tetap cocok).
  - `route_name` = `admin.{uri}` bila namespace berawal `Administrator`, selain itu `{uri}` (misal `admin.bagian.index`, `daftar_pasien.store`). **Nama route HARUS dipertahankan** karena semua blade/controller memanggil `route('admin.bagian.index')`, `route('daftar_pasien.create')`, dst.
- `SubMenuRouteServiceProvider` (`bootstrap/providers.php`) — di `$this->app->booted()` (sehabis web.php dimuat, jadi route eksplisit di web.php selalu menang), untuk tiap `sub_menu` aktif (status_batal != 1) yang `file_sub_menu`-nya bukan `'#'`:
  ...
  - Bila controller tidak ada (view statis/landing), daftarkan `Route::get('/{uri}', SubMenuViewController::class)` untuk path-view-nya (bila view `moduls.{path}.index|.form|.{path}` ada).
