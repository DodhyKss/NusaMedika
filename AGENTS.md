...
- Modul 6 "Administrator" drives CRUD pages for modul/menu/sub_menu/bagian/profesi/jabatan/pegawai/user/wilayah. Resource routes at the root URI equal to `file_sub_menu` (`/modul`, `/menu`, `/sub_menu`, `/bagian`, `/profesi`, `/jabatan`, `/pegawai`, `/user`, `/wilayah`) but route names stay `admin.modul.*`, `admin.menu.*`, `admin.sub_menu.*`, `admin.bagian.*`, `admin.profesi.*`, `admin.jabatan.*`, `admin.pegawai.*`, `admin.user.*`, `admin.wilayah.*`.
...
- Form pasien (tambah/edit) menyimpan hanya `kelurahan_id`; blok `.wilayah-cascade` berisi 4 `<select data-wilayah="..." data-url="...">` yang dimuat berjenjang dari `resources/js/app.js` (`initWilayahCascade`). Prefill edit dikirim dari controller sebagai variabel `$prefill` lalu dirender `data-prefill='@json($prefill)'` — **jangan** tulis array literal multi-elemen langsung di argumen `@json([...])`: `compileJson` memecah argumen dengan `explode(',')` sehingga array terpotong dan menghasilkan ParseError.
- `generateNoMr()` di `DataPasienController` menghitung `MAX(no_mr)+1` di **semua** baris pasien (tanpa filter `status_batal`) karena constraint `unique_no_mr` berlaku juga untuk record yang di-soft-delete.
