...
class DaftarRajalController extends Controller
{
    public function index()
    {
        ...
        $jadwalsByPoli = $jadwals->groupBy('bagian_id')->map(function ($group) {
            ...
            return $group->map(function ($j) use ($hariMap) {
                $waktuMulai = \Carbon\Carbon::parse($j->waktu_mulai)->format('H:i');
                $waktuSelesai = \Carbon\Carbon::parse($j->waktu_selesai)->format('H:i');
                return [
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $userDokter = \App\Models\User::where('pegawai_id', $dokter_id)->first();
