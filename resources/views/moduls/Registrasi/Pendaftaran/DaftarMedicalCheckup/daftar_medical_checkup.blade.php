...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Pendaftaran Medical Checkup</h1>
        <p class="text-sm text-slate-500 mt-1">Daftarkan pasien untuk layanan Medical Checkup (MCU).</p>
    </div>
    <div class="flex gap-2">
        <a href="{{ route('list_pasien_medical_checkup.index') }}" class="px-4 py-2 text-sm font-semibold text-slate-600 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors flex items-center gap-2 shadow-sm">
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <form action="{{ route('daftar_medical_checkup.store') }}" method="POST" id="formDaftarMcU">
        @csrf
        ...
        <div class="px-6 py-4 border-y border-slate-200 bg-slate-50/50 flex items-center gap-3 mt-2">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Tentukan poliklinik MCU dan jadwal dokter.</p>
            </div>
        </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <!-- Poliklinik MCU -->
                <div>
                    <label for="poliklinik" class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Poliklinik MCU <span class="text-red-500">*</span></label>
        ...
        <!-- Section 3: Penjamin & Informasi Tambahan -->
        <div class="px-6 py-4 border-y border-slate-200 bg-slate-50/50 flex items-center gap-3 mt-2">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Lengkapi data penjamin, surat keterangan, dan keluhan pasien.</p>
            </div>
        </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                    <label for="suket_mcu_id" class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Jenis Surat Keterangan</label>
                    <select id="suket_mcu_id" name="suket_mcu_id"
                            class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 appearance-none">
                        <option value="">-- Tidak Meminta Suket --</option>
                        @foreach ($sukets as $suket)
                            <option value="{{ $suket->suket_mcu_id }}" @selected((string) old('suket_mcu_id') === (string) $suket->suket_mcu_id)>
                                {{ $suket->nama_suket }} - Rp {{ number_format((float) $suket->harga, 0, ',', '.') }}
                            </option>
                        @endforeach
                    </select>
                    <p class="mt-1 text-[11px] text-slate-400">Opsional. Biaya suket diambil dari tarif saat pendaftaran.</p>
                </div>
                ...
                <div class="md:col-span-2">
                    <label for="keluhan" class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Keluhan / Alasan Kunjungan <span class="text-red-500">*</span></label>
                    <textarea id="keluhan" name="keluhan" rows="3" placeholder="Deskripsikan tujuan/keluhan utama pasien secara singkat..."
