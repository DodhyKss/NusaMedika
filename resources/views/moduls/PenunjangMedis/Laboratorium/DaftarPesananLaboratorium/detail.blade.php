...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    ...
    <div class="flex items-center gap-2 flex-wrap">
        @if (in_array($statusOrder, [0, 1], true))
            ...
            <form action="{{ route('order_laboratorium.batal', $order->order_laboratorium_id) }}" method="POST" onsubmit="return confirm('Yakin membatalkan order ini?');">
                @csrf
    ...
    <form action="{{ route('order_laboratorium.simpan_hasil', $order->order_laboratorium_id) }}" method="POST" id="formHasil">
        ...
        @if ($bisaUbah)
            <div class="px-5 py-4 border-t border-slate-100 flex items-center gap-2 justify-end">
                ...
                <form action="{{ route('order_laboratorium.selesai', $order->order_laboratorium_id) }}" method="POST" onsubmit="return confirm('Finalisasi order ini? Hasil yang belum diisi akan ditandai kosong.');">
                    @csrf
