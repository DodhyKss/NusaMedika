...
<div class="mb-6 flex justify-between items-end">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Daftar Pesanan Laboratorium</h1>
        <p class="text-sm text-slate-500 mt-1">Terima, entri hasil, dan selesaikan order laboratorium dari EMR berdasarkan bagian tempat Anda bertugas.</p>
    </div>
</div>

<!-- Pilih Bagian -->
<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-5">
    <form action="{{ route('daftar_pesanan_laboratorium.pilih_bagian') }}" method="POST" class="flex flex-col sm:flex-row sm:items-end gap-3">
        @csrf
        <div class="flex-1">
            <label for="bagian_tujuan_id" class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Bagian Laboratorium Aktif</label>
    ...
    @else
        <p class="mt-2 text-xs text-slate-400">Pilih bagian laboratorium terlebih dahulu sebelum mengelola pesanan.</p>
    @endif
</div>

<!-- Filter Pencarian -->
<form method="GET" action="{{ route('daftar_pesanan_laboratorium.index') }}" class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-5">
    ...
    <div class="flex items-center gap-2 mt-4">
        <a href="{{ route('daftar_pesanan_laboratorium.index') }}" title="Reset Filter" class="inline-flex items-center justify-center bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-700 text-sm font-semibold py-2.5 px-4 rounded-lg shadow-sm transition-colors">
            Reset
        </a>
        ...
        <table class="w-full text-left" style="min-width: 960px;">
            <thead>
                <tr class="bg-slate-50/60 border-b border-slate-100">
                    ...
                    <th class="px-3 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Pemeriksaan</th>
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-2">
                                @if ((int) $o->status_order === 0)
                                    <form action="{{ route('order_laboratorium.batal', $o->order_laboratorium_id) }}" method="POST" data-confirm-message="Yakin membatalkan order ini?" data-confirm-danger="true" data-confirm-title="Batalkan">
                                        @csrf
                                ...
                                @if ((int) $o->status_order === 1)
                                    <form action="{{ route('order_laboratorium.selesai', $o->order_laboratorium_id) }}" method="POST">
                                        @csrf
                                ...
                                @endif
                                <a href="{{ route('order_laboratorium.cetak', $o->order_laboratorium_id) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-300 hover:bg-slate-50 rounded-lg transition-colors">
                                    Cetak
                                </a>
                                <a href="{{ route('daftar_pesanan_laboratorium.detail', $o->order_laboratorium_id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-white bg-slate-600 hover:bg-slate-700 rounded-lg transition-colors">
                                    Detail
                                </a>
                            </div>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-5 py-12 text-center text-sm text-slate-400">
                            Tidak ada order laboratorium ditemukan untuk filter ini.
                        </td>
                    </tr>
                @endforelse
            </tbody>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm">
    <div class="flex flex-col items-center gap-3 py-16 text-slate-400">
        ...
        <p class="text-sm font-medium">Gunakan filter pencarian di atas untuk menampilkan daftar order laboratorium.</p>
    </div>
</div>
@endif
@endsection