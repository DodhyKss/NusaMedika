...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Detail Order Radiologi</h1>
        <p class="text-sm text-slate-500 mt-1">Terima order dan entri hasil pemeriksaan radiologi.</p>
    </div>
    <div class="flex items-center gap-2 flex-wrap">
        @if (in_array($statusOrder, [0, 1], true))
            @if ($statusOrder === 0)
                <form action="{{ route('order_radiologi.terima', $order->order_radiologi_id) }}" method="POST">
                    @csrf
            ...
            @endif
            <a href="{{ route('order_radiologi.cetak', $order->order_radiologi_id) }}" target="_blank" class="px-4 py-2 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm shadow-blue-600/20 transition-all flex items-center gap-2">
        ...
        @endif
        <a href="{{ route('daftar_pesanan_radiologi.index') }}" class="px-4 py-2 text-sm font-semibold text-slate-600 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors flex items-center gap-2 shadow-sm">
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-5">
    ...
    <form action="{{ route('order_radiologi.simpan_hasil', $order->order_radiologi_id) }}" method="POST" id="formHasil">
        @csrf
        <div class="overflow-x-auto drag-scroll">
            <table class="w-full text-left" style="min-width: 560px;">
                <thead>
                    <tr class="border-b border-slate-100">
                        <th class="px-5 py-2.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Tindakan</th>
                        <th class="px-3 py-2.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Hasil / Kesan</th>
                        <th class="px-3 py-2.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Abnormal</th>
                ...
                <tbody class="text-[13px] divide-y divide-slate-100">
                    @forelse ($order->details as $d)
                        <tr @if (!empty($d->hasil) && (int) $d->flag_abnormal === 1) class="bg-red-50/40" @endif>
                            ...
                            <td class="px-3 py-3">
                                <textarea name="hasil[{{ $d->order_radiologi_detail_id }}]" rows="2" {{ $bisaUbah ? '' : 'readonly' }}
                                          placeholder="Isi hasil / kesan..." class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2 {{ $bisaUbah ? 'bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500' : 'bg-slate-50 text-slate-500 cursor-not-allowed' }} outline-none transition-all">{{ old('hasil.'.$d->order_radiologi_detail_id, $d->hasil) }}</textarea>
                            </td>
                            <td class="px-3 py-3 text-center">
                                <input type="checkbox" name="flag_abnormal[{{ $d->order_radiologi_detail_id }}]" value="1"
