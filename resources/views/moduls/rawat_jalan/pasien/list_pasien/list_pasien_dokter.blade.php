...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
    ...
    <form action="{{ route('list_pasien_dokter.index') }}" method="GET" id="filterForm">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            ...
            <div class="flex justify-end gap-2 mt-2 md:mt-0">
                <a href="{{ route('list_pasien_dokter.index') }}" title="Reset Filter" class="inline-flex items-center justify-center bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-700 text-sm font-semibold py-2.5 px-4 rounded-lg shadow-sm transition-colors">
                    Reset
                </a>
                ...
                @forelse($listPasien as $idx => $row)
                    <tr class="hover:bg-blue-50/40 transition-colors">
                        ...
                        <td class="px-3 py-3 text-center">
                            ...
                            @elseif(in_array(env('FORM_ID_PENGKAJIAN_AWAL_KEPERAWATAN'), $forms) || in_array(env('FORM_ID_PENGKAJIAN_HARIAN_KEPERAWATAN'), $forms))
                        ...
                        <td class="px-3 py-3 text-center">
                            ...
                            @if((in_array(env('FORM_ID_PENGKAJIAN_AWAL_KEPERAWATAN'), $forms) || in_array(env('FORM_ID_PENGKAJIAN_HARIAN_KEPERAWATAN'), $forms)) && empty($row->status_selesai))
                                <a href="{{ route('dashboard_pasien.index', $row->registrasi_detail_id) }}" class="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-colors">
