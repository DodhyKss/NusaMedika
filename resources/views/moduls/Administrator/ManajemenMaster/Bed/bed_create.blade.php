...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Tambah Bed</h1>
        <p class="text-sm text-slate-500 mt-1">Tambahkan data bed / tempat tidur baru.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Lengkapi data bed / tempat tidur.</p>
        </div>
    </div>

    <!-- Form Body -->
    <div class="p-6">
        <form action="{{ route('admin.bed.store') }}" method="POST" id="formTambahBed">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                    ...
                    <select id="bagian_id" name="bagian_id"
                        class="select2-ruangan w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700"
                        required>
                        <option value=""></option>
                        @foreach ($bagians as $ruang)
                            <option value="{{ $ruang->bagian_id }}" @selected((string) old('bagian_id') === (string) $ruang->bagian_id)>{{ $ruang->nama_bagian }}</option>
                        @endforeach
                    </select>
                    @error('bagian_id')
                ...
                <div>
                    ...
                    <select id="kelas_id" name="kelas_id"
                        ...
                        @foreach ($kelasList as $kelas)
                            <option value="{{ $kelas->kelas_ruang_id }}" @selected((string) old('kelas_id') === (string) $kelas->kelas_ruang_id)>{{ $kelas->nama_kelas_ruang }}</option>
                        @endforeach
                    </select>
                    @error('kelas_id')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" id="no_kamar" name="no_kamar" value="{{ old('no_kamar') }}" placeholder="Contoh: R16-01"
                ...
                <div>
                    ...
                    <input type="text" id="nama_bed" name="nama_bed" value="{{ old('nama_bed') }}" placeholder="Contoh: B-01"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('nama_bed')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="flag_isolasi" name="flag_isolasi" class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700">
                        <option value="2" @selected(old('flag_isolasi', 2) == 2)>Normal</option>
                        <option value="1" @selected(old('flag_isolasi') == 1)>Isolasi</option>
                        <option value="3" @selected(old('flag_isolasi') == 3)>Covid-19</option>
                    </select>
                    @error('flag_isolasi')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
