...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Tambah Jadwal Dokter</h1>
        <p class="text-sm text-slate-500 mt-1">Tambahkan jadwal praktek dokter untuk pelayanan rawat jalan.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Lengkapi data dokter, poliklinik, dan jam praktek.</p>
        </div>
    </div>

    <!-- Form Body -->
    <div class="p-6">
        <form action="{{ route('admin.jadwal_dokter.store') }}" method="POST" id="formTambahJadwal">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">

                <!-- Dokter -->
                <div>
                    <x-select_dokter :selected="old('pegawai_id')" required />
                    @error('pegawai_id')
                ...
                <!-- Poliklinik -->
                <div>
                    <x-select_poliklinik name="bagian_id" id="bagian_id" :selected="old('bagian_id')" label="Poliklinik" placeholder="-- Pilih Poliklinik --" required />
                    @error('bagian_id')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="hari" name="hari" required
                        ...
                        @foreach ($hariMap as $kode => $namaHari)
                            <option value="{{ $kode }}" @selected((string) old('hari') === (string) $kode)>{{ $namaHari }}</option>
                        @endforeach
                    </select>
                    @error('hari')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" id="ruang_praktek" name="ruang_praktek" value="{{ old('ruang_praktek') }}" maxlength="10" placeholder="Contoh: Poli A"
                ...
                <div>
                    ...
                    <input type="number" id="kuota" name="kuota" value="{{ old('kuota') }}" min="0" required placeholder="Contoh: 20"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('kuota')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
