...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Edit Distributor</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui data distributor barang.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Perbaiki data identitas distributor barang.</p>
        </div>
    </div>

    <!-- Form Body -->
    <div class="p-6">
        <form action="{{ route('admin.distributor.update', $distributor->supplier_id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="nama_supplier" name="nama_supplier" value="{{ old('nama_supplier', $distributor->nama_supplier) }}" placeholder="Contoh: PT. Anugrah Prima" required
                ...
                <div>
                    ...
                    <select id="supplier_id" name="supplier_id" required
                        ...
                        @foreach ($supplierList as $s)
                            <option value="{{ $s->supplier_id }}" @selected((string) old('supplier_id', $selectedSupplierId) === (string) $s->supplier_id)>{{ $s->nama_supplier }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="telepon" name="telepon" value="{{ old('telepon', $distributor->telepon) }}" placeholder="Contoh: 021-5551234"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('telepon')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $distributor->email) }}" placeholder="Contoh: cs@distributor.co.id"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('email')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="npwp" name="npwp" value="{{ old('npwp', $distributor->npwp) }}" placeholder="Contoh: 02.111.222.3-444.000"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('npwp')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-5">
                ...
                <textarea id="alamat" name="alamat" rows="2" placeholder="Alamat lengkap distributor..."
                          class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400 resize-none">{{ old('alamat', $distributor->alamat) }}</textarea>
                @error('alamat')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
