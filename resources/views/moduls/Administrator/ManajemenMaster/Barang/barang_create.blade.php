...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Tambah Barang</h1>
        <p class="text-sm text-slate-500 mt-1">Tambahkan data barang / obat baru ke master.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Lengkapi data barang, satuan, racikan, dan jenis fornas.</p>
        </div>
    </div>

    <!-- Form Body -->
    <div class="p-6">
        <form action="{{ route('admin.barang.store') }}" method="POST" id="formTambahBarang">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="kode_barang" name="kode_barang" value="{{ old('kode_barang') }}" placeholder="Contoh: OBAT-0001"
                ...
                <div>
                    ...
                    <input type="text" id="nama_barang" name="nama_barang" value="{{ old('nama_barang') }}" placeholder="Contoh: Paracetamol 500 mg" required
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('nama_barang')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="jenis_barang_id" name="jenis_barang_id" required
                        ...
                        @foreach ($barangJenisList as $jenis)
                            <option value="{{ $jenis->barang_jenis_id }}" @selected((string) old('jenis_barang_id') === (string) $jenis->barang_jenis_id)>{{ $jenis->nama_jenis_barang }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="satuan_id" name="satuan_id"
                        ...
                        @foreach ($satuanList as $s)
                            <option value="{{ $s->satuan_id }}" @selected((string) old('satuan_id') === (string) $s->satuan_id)>{{ $s->nama_satuan }}</option>
                        @endforeach
                    </select>
                    @error('satuan_id')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="is_racikan" name="is_racikan"
                            class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700">
                        <option value="0" @selected(old('is_racikan', 0) === 0 || old('is_racikan') === '0')>Tidak</option>
                        <option value="1" @selected((string) old('is_racikan') === '1')>Ya</option>
                    </select>
                    @error('is_racikan')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="is_fornas" name="is_fornas"
                            class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700">
                        <option value="0" @selected(old('is_fornas', 0) === 0 || old('is_fornas') === '0')>Tidak / Non Fornas</option>
                        <option value="1" @selected((string) old('is_fornas') === '1')>Fornas Nasional</option>
                        <option value="2" @selected((string) old('is_fornas') === '2')>Fornas Rumah Sakit</option>
                    </select>
                    @error('is_fornas')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
