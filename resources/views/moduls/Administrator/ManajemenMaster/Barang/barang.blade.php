...
<div class="mb-6 flex justify-between items-end">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Master Barang</h1>
        <p class="text-sm text-slate-500 mt-1">Kelola data barang / obat (satuan, racikan & jenis fornas). Harga dikelola per batch di menu Harga Barang.</p>
    </div>
    <div class="flex gap-2">
        <a href="{{ route('admin.barang.create') }}" class="px-4 py-2.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm shadow-blue-600/20 transition-all hover:-translate-y-0.5 flex items-center gap-2">
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
    ...
    <form action="{{ route('admin.barang.index') }}" method="GET" id="filterForm">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div class="col-span-1 md:col-span-3">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="search" name="search" value="{{ old('search', $search) }}" placeholder="Cari Kode / Nama Barang / Jenis..."
            ...
            <div class="flex justify-end gap-2 mt-2 md:mt-0">
                <a href="{{ route('admin.barang.index') }}" title="Reset Filter" class="inline-flex items-center justify-center bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-700 text-sm font-semibold py-2.5 px-4 rounded-lg shadow-sm transition-colors">
                    Reset
                </a>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-left" style="min-width: 1100px;">
            ...
            <tbody class="text-[12px] divide-y divide-slate-100">
                @forelse ($barangList as $i => $b)
                    @php
                        $noUrut = ($barangList->firstItem() ?? 0) + $i;
                    @endphp
                    <tr class="hover:bg-blue-50/40 transition-colors">
                        <td class="px-3 py-3 text-center text-slate-500">{{ $noUrut }}</td>
                        <td class="px-3 py-3 text-slate-600">{{ $b->kode_barang ?: '-' }}</td>
                        <td class="px-3 py-3 font-semibold text-slate-800">{{ $b->nama_barang }}</td>
                        <td class="px-3 py-3 text-slate-600">{{ $b->jenis->nama_jenis_barang ?? '-' }}</td>
                        <td class="px-3 py-3 text-center text-slate-600">{{ $b->textSatuan() }}</td>
                        <td class="px-3 py-3 text-center">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold {{ $b->is_racikan ? 'bg-violet-100 text-violet-700' : 'bg-slate-100 text-slate-500' }}">
                                {{ $b->racikan_label }}
                            </span>
                        </td>
                        <td class="px-3 py-3 text-center">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold {{ $b->is_fornas ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">
                                {{ $b->fornas_label }}
                            </span>
                        </td>
                        <td class="px-3 py-3 text-center">
                        ...
                        <td class="px-3 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <form action="{{ route('admin.barang.destroy', $b->barang_id) }}" method="POST" data-confirm-message="Yakin ingin menghapus barang ini?" data-confirm-danger="true" data-confirm-title="Hapus Data" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="9" class="px-3 py-12 text-center">
                            <div class="flex flex-col items-center gap-2 text-slate-400">
                                ...
                                <p class="text-xs">Klik tombol "Tambah Barang" untuk menambahkan data pertama.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    <div class="px-5 py-3.5 border-t border-slate-100 bg-slate-50/50">
        {{ $barangList->withQueryString()->links('components.pagination') }}
    </div>
</div>
@endsection