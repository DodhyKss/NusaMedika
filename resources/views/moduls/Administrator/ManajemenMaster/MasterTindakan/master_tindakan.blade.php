...
<div class="mb-6 flex justify-between items-end">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Master Tindakan</h1>
        <p class="text-sm text-slate-500 mt-1">Katalog tindakan penunjang (Laboratorium & Radiologi) beserta tarif per kelas perawatan.</p>
    </div>
    <div class="flex gap-2">
        <a href="{{ route('admin.master_tindakan.create') }}" class="px-4 py-2.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm shadow-blue-600/20 transition-all hover:-translate-y-0.5 flex items-center gap-2">
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-6">
    ...
    <form action="{{ route('admin.master_tindakan.index') }}" method="GET" id="filterForm">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
            ...
            <div class="col-span-1 md:col-span-3">
                ...
                <select id="bagian_id" name="bagian_id" class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700">
                    ...
                    @foreach ($bagianList as $b)
                        <option value="{{ $b->bagian_id }}" @selected((int) $bagianId === (int) $b->bagian_id)>{{ $b->nama_bagian }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-span-1 md:col-span-2">
                <label for="kategori" class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Kategori</label>
                <select id="kategori" name="kategori" class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700">
                    <option value="">Semua</option>
                    @foreach (\App\Models\Tindakan::KATEGORI as $val => $label)
            ...
            <div class="col-span-1 md:col-span-2 flex justify-end gap-2 mt-2 md:mt-0">
                <a href="{{ route('admin.master_tindakan.index') }}" class="inline-flex items-center justify-center bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-700 text-sm font-semibold py-2.5 px-4 rounded-lg shadow-sm transition-colors">Reset</a>
        ...
        <table class="w-full text-left" style="min-width: 900px;">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-200">
                    ...
                    <th class="px-3 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Satuan / Nilai Normal</th>
                    <th class="px-3 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider text-right">Tarif Default</th>
            ...
            <tbody class="text-[12px] divide-y divide-slate-100">
                @forelse ($tindakanList as $i => $t)
                    ...
                    <tr class="hover:bg-blue-50/40 transition-colors">
                        ...
                        <td class="px-3 py-3 text-slate-600">{{ $t->bagian?->nama_bagian ?? '-' }}</td>
                        <td class="px-3 py-3 text-center">
                            <span class="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold {{ $t->kategori === 'LAB' ? 'bg-violet-100 text-violet-700' : ($t->kategori === 'RAD' ? 'bg-cyan-100 text-cyan-700' : 'bg-slate-100 text-slate-600') }}">{{ $t->kategori_label }}</span>
                        </td>
                        <td class="px-3 py-3 text-slate-600">{{ $t->satuan_hasil ? ($t->satuan_hasil.($t->nilai_normal ? ' · '.$t->nilai_normal : '')) : ($t->nilai_normal ?? '-') }}</td>
                        <td class="px-3 py-3 text-right font-semibold text-slate-700">{{ $tarifDefault ? number_format((float) $tarifDefault->tarif, 0, ',', '.') : '-' }}</td>
                        <td class="px-3 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('admin.master_tindakan.edit', $t->tindakan_id) }}" class="cursor-pointer p-1.5 text-blue-500 hover:bg-blue-50 hover:text-blue-600 rounded-md transition-colors" title="Edit Data">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <form action="{{ route('admin.master_tindakan.destroy', $t->tindakan_id) }}" method="POST" data-confirm-message="Yakin ingin menghapus tindakan ini?" data-confirm-danger="true" data-confirm-title="Hapus Data" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="8" class="px-3 py-12 text-center">
                            <div class="flex flex-col items-center gap-2 text-slate-400">
                                ...
                                <p class="text-sm font-medium">Belum ada data tindakan.</p>
                                <p class="text-xs">Klik tombol "Tambah Tindakan" untuk menambahkan data pertama.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
