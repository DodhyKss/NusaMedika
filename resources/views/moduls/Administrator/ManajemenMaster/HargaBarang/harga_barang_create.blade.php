...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Tambah Harga Barang</h1>
        <p class="text-sm text-slate-500 mt-1">Tentukan harga beli & jual untuk satu no. batch barang.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Pilih barang, isi no. batch, dan harga beli/jual.</p>
        </div>
    </div>

    <!-- Form Body -->
    <div class="p-6">
        <form action="{{ route('admin.harga_barang.store') }}" method="POST" id="formTambahHarga">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select id="barang_id" name="barang_id"
                        ...
                        @foreach ($barangList as $b)
                            <option value="{{ $b->barang_id }}" @selected((string) old('barang_id') === (string) $b->barang_id)>
                ...
                <div>
                    ...
                    <input type="text" id="no_batch" name="no_batch" value="{{ old('no_batch') }}" placeholder="mis. B001" maxlength="50"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="harga_beli" name="harga_beli" value="{{ old('harga_beli') }}" placeholder="0" inputmode="numeric"
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-semibold">Rp</span>
                        <input type="text" id="harga_jual" name="harga_jual" value="{{ old('harga_jual') }}" placeholder="0" inputmode="numeric"
                               class="w-full text-sm border border-slate-200 rounded-lg pl-9 pr-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    </div>
                    @error('harga_jual')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('formTambahHarga').addEventListener('submit', function () {
