...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Edit Supplier</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui data supplier / rekanan penyedia.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Perbaiki data identitas rekanan penyedia barang.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.supplier.update', $supplier->supplier_id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="nama_supplier" name="nama_supplier" value="{{ old('nama_supplier', $supplier->nama_supplier) }}" placeholder="Contoh: PT. Medika Sejahtera" required
                ...
                <div>
                    ...
                    <input type="text" id="telepon" name="telepon" value="{{ old('telepon', $supplier->telepon) }}" placeholder="Contoh: 081234567890"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('telepon')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $supplier->email) }}" placeholder="Contoh: sales@medika.co.id"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('email')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="npwp" name="npwp" value="{{ old('npwp', $supplier->npwp) }}" placeholder="Contoh: 01.234.567.8-901.000"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('npwp')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-5">
                ...
                <textarea id="alamat" name="alamat" rows="2" placeholder="Alamat lengkap supplier..."
                          class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400 resize-none">{{ old('alamat', $supplier->alamat) }}</textarea>
                @error('alamat')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
