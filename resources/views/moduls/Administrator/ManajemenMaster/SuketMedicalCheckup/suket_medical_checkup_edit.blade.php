...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Edit Suket Medical Checkup</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui data surat keterangan medical checkup.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Perbaiki data surat keterangan medical checkup.</p>
        </div>
    </div>

    <!-- Form Body -->
    <div class="p-6">
        <form action="{{ route('admin.suket_medical_checkup.update', $suket->suket_mcu_id) }}" method="POST" id="formEditSuket">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="nama_suket" name="nama_suket" value="{{ old('nama_suket', $suket->nama_suket) }}" placeholder="Contoh: Surat Keterangan Sehat"
                ...
                <div>
                    ...
                    <input type="number" id="harga" name="harga" value="{{ old('harga', $suket->harga) }}" placeholder="Contoh: 50000" min="0" step="0.01"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                    @error('harga')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
