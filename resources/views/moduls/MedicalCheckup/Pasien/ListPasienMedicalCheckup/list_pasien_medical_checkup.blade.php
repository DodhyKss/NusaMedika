...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">List Pasien Medical Checkup</h1>
        <p class="text-sm text-slate-500 mt-1">Silahkan lakukan pencarian terlebih dahulu untuk menampilkan data pasien MCU.</p>
    </div>
    <div class="flex gap-2">
        <a href="{{ route('daftar_medical_checkup.index') }}" class="px-4 py-2.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm shadow-blue-600/20 transition-all hover:-translate-y-0.5 flex items-center gap-2">
...
<div class="bg-white rounded-xl border-slate-200 shadow-sm p-5 mb-6">
    ...
    <form action="{{ route('list_pasien_medical_checkup.index') }}" method="GET" id="filterForm">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div>
                <label for="poliklinik" class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Poliklinik MCU</label>
                <select id="poliklinik" name="poliklinik"
                        class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 appearance-none @error('poliklinik') border-red-400 @enderror">
                    <option value="">-- Pilih Poliklinik MCU --</option>
                    @foreach ($polikliniks as $poli)
                        <option value="{{ $poli->bagian_id }}" @selected((string) $poliklinikId === (string) $poli->bagian_id)>{{ $poli->nama_bagian }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div class="flex justify-end gap-2 mt-2 md:mt-0">
                <a href="{{ route('list_pasien_medical_checkup.index') }}" title="Reset Filter" class="inline-flex items-center justify-center bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-700 text-sm font-semibold py-2.5 px-4 rounded-lg shadow-sm transition-colors">
                    Reset
                </a>
        ...
        <table class="w-full text-left" style="min-width: 1100px;">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-200">
                    ...
                    <th class="px-3 py-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Jenis Suket</th>
                        ...
                        <td class="px-3 py-3 font-semibold text-slate-800">
                            ...
                            <span class="block text-[10px] text-slate-400 font-normal mt-0.5">
                                @if($row->tgl_lahir && $row->tgl_lahir != '0000-00-00')
                                    {{ \Carbon\Carbon::parse($row->tgl_lahir)->age }} Tahun
                                @else
                                    -
                                @endif
                            </span>
                        ...
                        <td class="px-3 py-3">
                            @if($row->nama_suket)
                                <span class="inline-flex flex-col gap-0.5">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] font-bold bg-purple-100 text-purple-700">
                                        {{ $row->nama_suket }}
                                    </span>
                                    <span class="text-[10px] text-slate-500">Rp {{ number_format((float) $row->harga_suket, 0, ',', '.') }}</span>
                                </span>
                            @else
                    ...
                    <tr>
                        <td colspan="9" class="px-6 py-12 text-center text-slate-400">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <span class="text-sm font-medium">Tidak ada data antrean pasien MCU untuk filter yang dipilih</span>
                            </div>
                        </td>
                    </tr>
                @endforelse
