...
@section('content')
    @php
        ...
        $actionUrl = $isEdit
            ? route('emr.form.update', ['form_name' => 'radiologi', 'registrasi_detail_id' => $registrasi_detail->registrasi_detail_id, 'emr_id' => $edit->order_radiologi_id])
            : route('emr.form.store', ['form_name' => 'radiologi', 'registrasi_detail_id' => $registrasi_detail->registrasi_detail_id]);

        $deleteUrl = $isEdit
            ? route('emr.form.destroy', ['form_name' => 'radiologi', 'registrasi_detail_id' => $registrasi_detail->registrasi_detail_id, 'emr_id' => $edit->order_radiologi_id])
            : '';

        $aksesCrud = $aksesCrud ?? ['create' => true, 'read' => true, 'update' => true, 'delete' => true];

        $titleForm = 'Order Radiologi Baru';
        $subtitleForm = 'Perintahkan pemeriksaan radiologi untuk pasien.';
        if ($isEdit) {
            $titleForm = 'Edit Order Radiologi';
            $subtitleForm = 'Perbarui order radiologi yang masih menunggu.';
        } elseif ($isView) {
            $titleForm = 'Detail Order Radiologi';
    ...
    >
        <x-slot name="listRiwayat">
            ...
            @forelse ($orders as $order)
                @php
                    $isActive = isset($edit) && $edit && $edit->order_radiologi_id == $order->order_radiologi_id;
                ...
                <div class="riwayat-item bg-white rounded-xl border {{ $isActive ? 'border-blue-400 ring-2 ring-blue-500/10' : 'border-slate-200' }} shadow-sm overflow-hidden mb-3">
                    ...
                    <div class="px-4 py-2 border-t border-slate-100 flex items-center gap-1 justify-end">
                        @if ($aksesCrud['read'])
                            <a href="{{ url('emr/form/radiologi/'.$registrasi_detail->registrasi_detail_id.'/'.$order->order_radiologi_id.'?action=view') }}" class="p-1.5 text-slate-600 hover:bg-slate-100 rounded-md transition-colors" title="Lihat">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                            </a>
                        @endif
                        @if ($aksesCrud['update'] && (int) $order->status_order === 0)
                            <a href="{{ url('emr/form/radiologi/'.$registrasi_detail->registrasi_detail_id.'/'.$order->order_radiologi_id) }}" onclick="document.getElementById('save_loader').classList.remove('hidden'); document.querySelector('#save_loader div:nth-child(2)').innerText='Memuat Data...';" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-md transition-colors" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                        @endif
                        @if ($aksesCrud['delete'] && (int) $order->status_order === 0)
                            <form action="{{ route('emr.form.destroy', ['form_name' => 'radiologi', 'registrasi_detail_id' => $registrasi_detail->registrasi_detail_id, 'emr_id' => $order->order_radiologi_id]) }}" method="POST" data-confirm-message="Apakah Anda yakin ingin membatalkan order ini?" data-confirm-danger="true" data-confirm-title="Batalkan" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div class="p-5 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="prioritas" name="prioritas" {{ $isView ? 'disabled' : '' }} class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 bg-slate-50 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-slate-700">
                            @foreach (\App\Models\OrderRadiologi::PRIORITAS as $prio)
                        ...
                        <table class="w-full text-left" style="min-width: 520px;">
                            ...
                            <tbody id="tbodyItem" class="divide-y divide-slate-100">
                                <tr id="rowPlaceholder" class="text-center">
                                    <td colspan="2" class="px-3 py-6 text-xs text-slate-400">Belum ada item. Klik "Tambah Item" untuk memilih pemeriksaan.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                @if ($isView && (int) $edit->status_order === 2)
                    <div>
                        <h3 class="text-sm font-semibold text-slate-800 mb-3">Deskripsi Hasil</h3>
                        <div class="overflow-x-auto rounded-lg border border-slate-200">
                            <table class="w-full text-left">
                                <thead>
                                    <tr class="bg-slate-50 border-b border-slate-200">
                                        <th class="px-3 py-2.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Pemeriksaan</th>
                                        <th class="px-3 py-2.5 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Hasil / Kesan</th>
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @foreach ($edit->details as $dtl)
                                        <tr>
                                            ...
                                            <td class="px-3 py-2.5 {{ $dtl->flag_abnormal ? 'font-bold text-red-600' : 'text-slate-700' }} whitespace-pre-line">{{ $dtl->hasil ?? '-' }}</td>
