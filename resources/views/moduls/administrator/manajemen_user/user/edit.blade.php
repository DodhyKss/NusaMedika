...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Edit User</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui data user dan hak aksesnya.</p>
    </div>
...
<form action="{{ route('admin.user.update', $user->user_id) }}" method="POST" id="formEditUser">
    @csrf
    @method('PUT')
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="user_password" name="user_password" value="{{ old('user_password') }}" placeholder="Kosongkan jika tidak diganti"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                ...
                <div>
                    ...
                    <select id="pegawai_id" name="pegawai_id"
                        ...
                        @foreach ($pegawais as $pegawai)
                            <option value="{{ $pegawai->pegawai_id }}" data-nama="{{ $pegawai->nama_pegawai }}" {{ old('pegawai_id', $user->pegawai_id) == $pegawai->pegawai_id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" id="nama_pegawai" name="nama_pegawai" value="{{ old('nama_pegawai', $user->nama_pegawai) }}"
                           class="w-full text-sm border border-slate-200 rounded-lg px-3.5 py-2.5 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-700 placeholder-slate-400">
                                        ...
                                        <div class="mb-4 last:mb-0">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                                                @foreach ($menu->subMenus as $subMenu)
                                                    @php
                                                        $isChecked = in_array($subMenu->sub_menu_id, old('sub_menu_ids', $userAkses));
                                                    @endphp
...
<script>
    function toggleSemua(checked) {
        document.querySelectorAll('#formEditUser input[name="sub_menu_ids[]"]').forEach(function (cb) {
            cb.checked = checked;
        });
    }
