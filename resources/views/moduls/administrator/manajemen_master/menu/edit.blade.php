...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Edit Menu</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui data menu.</p>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="p-6">
        <form action="{{ route('admin.menu.update', $menu->menu_id) }}" method="POST" id="formEditMenu">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                <div>
                    ...
                    <select id="modul_id" name="modul_id"
                        ...
                        @foreach ($moduls as $modul)
                            <option value="{{ $modul->modul_id }}" {{ old('modul_id', $menu->modul_id) == $modul->modul_id ? 'selected' : '' }}>{{ $modul->nama_modul }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" id="urutan_menu" name="urutan_menu" value="{{ old('urutan_menu', $menu->urutan_menu) }}"
