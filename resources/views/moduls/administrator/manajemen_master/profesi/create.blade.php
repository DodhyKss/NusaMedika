...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-[22px] font-bold text-slate-900 tracking-tight">Tambah Profesi</h1>
        <p class="text-sm text-slate-500 mt-1">Buat data profesi baru.</p>
    </div>
        ...
        <form action="{{ route('admin.profesi.store') }}" method="POST" id="formTambahProfesi">
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-5">
                <div>
                    ...
                    <input type="text" id="nama_profesi" name="nama_profesi" value="{{ old('nama_profesi') }}" placeholder="Contoh: Dokter"
                ...
                <div>
                    <label class="flex items-center gap-2.5 cursor-pointer">
                        <input type="checkbox" name="ehr" value="1" {{ old('ehr') ? 'checked' : '' }}
