...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR REPLACE VIEW \"upload_fpk_view\" AS SELECT (arr.item_object ->> 'SEP'::text) AS sep,
    (arr.item_object ->> 'nilai_klaim'::text) AS nilai_klaim,
    (arr.item_object ->> 'klaim_dibayar'::text) AS klaim_dibayar,
    'SEP'::text AS jenis_id,
    upload_fpk.jenis_rawat
   FROM upload_fpk,
...
UNION
 SELECT (arr.item_object ->> 'CCN'::text) AS sep,
  ...
  GROUP BY (arr.item_object ->> 'CCN'::text), (arr.item_object ->> 'nilai_klaim'::text), (arr.item_object ->> 'klaim_dibayar'::text), upload_fpk.jenis_rawat;");
    }
    ...
    {
        DB::statement("DROP VIEW IF EXISTS \"upload_fpk_view\"");
    }
};
