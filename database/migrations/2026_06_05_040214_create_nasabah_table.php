...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('nasabah', function (Blueprint $table) {
            $table->integer('nasabah_id')->primary();
            $table->timestamp('input_time', 6)->nullable();
            $table->integer('input_user_id')->nullable();
            $table->timestamp('mod_time', 6)->nullable();
            $table->integer('mod_user_id')->nullable();
            $table->smallInteger('status_batal')->nullable()->index('nasabah_status_batal_idx');
            $table->string('nama_nasabah')->nullable()->index('nasabah_nama_nasabah_idx');
            $table->string('jenis_nasabah', 20)->nullable()->index('nasabah_jenis_nasabah_idx')->comment('PRIBADI                     				= PRI
