...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE VIEW \"bloat_index_report\" AS WITH index_info AS (
 ...
 SELECT index_info.schema_name,
    index_info.table_name,
    index_info.index_name,
    pg_size_pretty(index_info.index_size_bytes) AS index_size,
    round(
        CASE
  ...
  ORDER BY (round(
        CASE
            WHEN ((index_info.num_pages = 0) OR (index_info.num_rows = 0)) THEN (0)::numeric
            ELSE ((1)::numeric - (((index_info.num_rows)::numeric * 8.0) / ((index_info.num_pages * (current_setting('block_size'::text))::integer))::numeric))
        END, 3)) DESC, index_info.index_size_bytes DESC;");
    }
