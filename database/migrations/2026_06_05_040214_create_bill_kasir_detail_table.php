...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('bill_kasir_detail', function (Blueprint $table) {
            $table->integer('bill_kasir_detail_id')->primary();
            $table->timestamp('input_time', 6)->nullable();
            $table->integer('input_user_id')->nullable();
            $table->timestamp('mod_time', 6)->nullable();
            $table->integer('mod_user_id')->nullable();
            $table->smallInteger('status_batal')->nullable();
            $table->integer('bill_kasir_id')->nullable()->index('bill_kasir_detail_bill_kasir_id_idx');
            $table->integer('jenis_tindakan_id')->nullable();
            $table->integer('tarif_id')->nullable()->index('bill_kasir_detail_tarif_id_idx');
